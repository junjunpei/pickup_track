class Api::TracksController < ApplicationController
  ...
  def recommend
    ...
    http.use_ssl = true
    header = { 'Authorization' => "Bearer #{@bearer_token}", 'Accept-Language' => 'ja' }
    uri.query = URI.encode_www_form({ seed_artists: @track.artist_id, seed_genres: 'genres', seed_tracks: @track.track_id, limit: 20, market: 'JP', target_acousticness: @audio_features['acousticness'], target_danceability: @audio_features['danceability'], target_energy: @audio_features['energy'], target_instrumentalness: @audio_features['instrumentalness'], target_key: @audio_features['key'], target_loudness: @audio_features['loudness'], target_mode: @audio_features['mode'], target_speechiness: @audio_features['speechiness'], target_tempo: @audio_features['tempo'], target_time_signature: @audio_features['time_signature'], target_valence: @audio_features['valence'] })
    response = http.get(uri.request_uri, header)
    response_body = JSON.parse(response.body)
